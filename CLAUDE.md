...
- The Accesa **`de-storage`** module encapsulates the simulated Secure Element for offline DE tokens — token storage, holderPub keypair generation, `txCounter`. Treated like a real SE: no callers outside `de-feature` reach in. State persists under the existing Tink-AEAD `PrefsController` (so the master key is shared with the rest of the wallet's encrypted prefs). The module is recognised as UI-free by the convention plugin via an explicit override in `LibraryPluginConfig.kt`'s `isLogicModule` predicate.
- **Eurosystem trust roots are bundled**, not configured per build. `resources-logic/src/main/res/raw/accesa_workshop_ca.pem` is the workshop bank CA (OID4VP request signing); `workshop_ncb_root.pem` is the workshop NCB root (offline-token signature verification). Both are populated verbatim from `services/<service>/src/main/resources/...` in the companion repo and refreshed when the backend agent rotates them — see the PR12 → PR13 pattern. Bank-app URLs are never bundled (they arrive at runtime via `eudi-de-authorize://` deep-link params).
